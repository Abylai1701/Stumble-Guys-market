...
class LocationsViewController_e0Q04: BaseSectionViewController_e0Q04 {
    ...
    @objc override func backButtonTapped_e0Q04() {
        ...
        let menuVC = HomeViewController_e0Q04(selectedMenu: 5)
        menuVC.popAction = {
            let vc = self
            vc.removeFromParent()
    ...
    private func configureCell(_ cell: ModsCardCell_e0Q04, at indexPath: IndexPath) {
        let loacation = viewModel.items[indexPath.row]
        
        cell.titleLabel.text = loacation.title
        cell.descriptionLabel.text = loacation.description
        ...
        if let imageUrl = URL(string: loacation.image) {
            cell.previewImage.sd_setImage(with: imageUrl, placeholderImage: nil)
        }
        
        let favoriteImageName = viewModel.isModFavorite(loacation) ? Assets.Icon.favorite2 : Assets.Icon.favorite1
    ...
    open func handleCellTap(at indexPath: IndexPath) {
        ...
        let loacationItem = viewModel.items[indexPath.row]
        viewModel.modItem = loacationItem
        ...
        let singleLocationVC = SingleLocationViewController_e0Q04()
        singleLocationVC.viewModel = viewModel
        navigationController?.pushViewController(singleLocationVC, animated: false)
    }
    ...
    @objc private func downloadButtonTapped(_ sender: UIButton) {
        ...
        let modPath = viewModel.items[locationIndex].modPath
...
extension LocationsViewController_e0Q04 {
...
extension LocationsViewController_e0Q04: ViewModelDelegate {
...
extension LocationsViewController_e0Q04: UICollectionViewDataSource, UICollectionViewDelegate {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        if viewModel.items.isEmpty {
            ...
            guard let locationCell = collectionView.dequeueReusableCell(withReuseIdentifier: Cell.modsCard, for: indexPath) as? ModsCardCell_e0Q04 else {
                fatalError("Cannot create a new ModCell")
            }
            configureCell(locationCell, at: indexPath)
            return locationCell
        }
    }
...
extension LocationsViewController_e0Q04: UICollectionViewDelegateFlowLayout {
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        if device.isOneOf([.iPadMini, .iPadMini2, .iPadMini3, .iPadMini4, .iPadMini5, .iPadMini6,
            ...
            heightForItem = viewModel.items.isEmpty ? 0 : 280 // Example height for non-iPad devices, adjust as needed
        }
